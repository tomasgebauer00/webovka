...
export default function AdminPage() {
  ...
  // Ukládáme roli, abychom věděli, jestli jsi Super Admin
  ...
  // HLAVNÍ FUNKCE PRO ZMĚNU ROLÍ
  const changeUserRole = async (userId: string, newRole: string) => {
      if (!confirm(`Opravdu změnit roli uživatele na "${newRole.toUpperCase()}"?`)) return;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 border-b border-white/10 pb-4 overflow-x-auto">
          ...
          {userRole === 'super_admin' && (
             <button onClick={() => setActiveTab('users')} className={`px-6 py-2 rounded-full font-bold transition ${activeTab === 'users' ? 'bg-purple-600' : 'bg-slate-800'}`}>👥 Správa Rolí</button>
          )}
            ...
            <div className="bg-slate-900 p-6 rounded-2xl border border-white/10 h-fit">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="bg-slate-950 p-3 rounded border border-white/5 space-y-2">
                    ...
                    <div className="flex flex-col"><label className="text-[10px] text-slate-500">Původní cena</label><input type="number" name="original_price" value={formData.original_price} onChange={handleChange} className="bg-slate-800 border border-white/10 rounded p-2 text-red-300" /></div>
                ...
                {deals.map(deal => (
                    <div key={deal.id} className={`bg-slate-900 border p-4 rounded-xl flex justify-between items-center transition ${deal.is_special_offer ? 'border-blue-500 shadow-lg shadow-blue-500/20' : 'border-white/5'}`}>
                        <div className="flex gap-4 items-center">
                            ...
                            <div><h3 className="font-bold flex items-center gap-2">{deal.destination} {deal.is_special_offer && <span className="bg-blue-600 text-[10px] px-2 py-0.5 rounded text-white">AKČNÍ</span>}</h3><p className="text-sm text-green-400 font-bold">{deal.total_price} Kč</p></div>
                        </div>
                        <div className="flex gap-2"><button onClick={() => handleEdit(deal)} className="text-yellow-500 bg-yellow-500/10 p-2 rounded hover:bg-yellow-500/20">✏️</button><button onClick={() => handleDelete(deal.id)} className="text-red-500 bg-red-500/10 p-2 rounded hover:bg-red-500/20">🗑️</button></div>
                    </div>
                ))}
            </div>
          </div>
            ...
            <div className="space-y-4">
                {bookings.map(b => (
                    <div key={b.id} className="bg-slate-900 p-4 rounded-xl border border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
                        <div><h3 className="font-bold text-green-400">{b.deals?.destination}</h3><p className="text-sm text-slate-300">{b.name} ({b.email})</p><p className="text-xs text-slate-500">Osob: {b.people_count} | Cena: {b.total_price} Kč</p></div>
                        <div className="flex items-center gap-4"><button onClick={() => handleBookingStatus(b.id, 'confirmed')} className="bg-green-600 hover:bg-green-500 text-white p-2 rounded-lg text-xs font-bold">✅ Potvrdit</button><button onClick={() => handleBookingStatus(b.id, 'cancelled')} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded-lg text-xs font-bold">❌ Zamítnout</button></div>
                    </div>
                ))}
            </div>
        )}
        
        {/* SEKCE UŽIVATELÉ - TADY BUDOU TY TLAČÍTKA */}
          ...
          <div className="grid gap-4">
            {users.map(u => (
                <div key={u.id} className="border border-white/5 p-4 rounded-xl flex justify-between items-center bg-slate-900">
                    ...
                    {/* OCHRANA: Sebe (Super Admina) si nemůžeš upravit */}
                    {u.email !== 'triphack@outlook.cz' && (
                        <div className="flex flex-wrap gap-2">
                            {/* Tlačítka se zobrazují dynamicky podle toho, co uživatel NENÍ */}
                            
                            {/* 1. Udělat ADMINEM (Správce webu) */}
                            {u.role !== 'admin' && (
                            ...
                            {/* 2. Udělat PROVIDEREM (Hostitel / Půjčovna) */}
                            {u.role !== 'provider' && (
                ...
                {requests.map(req => (
                    <div key={req.id} className="bg-slate-900 p-6 rounded-xl border border-orange-500/20 flex flex-col gap-3">
                        ...
                        <div className="mt-2 border-t border-white/5 pt-3">
                             <button onClick={() => setChatRequestId(chatRequestId === req.id ? null : req.id)} className={`w-full py-2 rounded-lg font-bold transition flex items-center justify-center gap-2 ${chatRequestId === req.id ? 'bg-red-600 text-white' : 'bg-blue-600 hover:bg-blue-500 text-white'}`}>{chatRequestId === req.id ? 'Zavřít chat' : '💬 Otevřít chat'}</button>
